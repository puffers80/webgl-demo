//3DVolume
var colors=[
	vec3(1.0, 1.0, 0.0), vec3(0.55, 0.55, 0.55),
	vec3(0.7, 0.7, 0.0), vec3(0.0, 0.0, 0.0),
	vec3(0.7, 0.7, 0.0), vec3(1.0, 1.0, 0.0), 
	vec3(0.55, 0.55, 0.55)
	];
var attributes = [];
...
function divideTe(a, b, c, d, k){
	...
	if(k > 0){
		...
		mid[4] = mult(0.5, add(c, d));
		mid[5] = mult(0.5, add(b, d));
		divideTe(a, mid[0], mid[1], mid[2], k-1);
		divideTe(mid[0], b, mid[3], mid[5], k-1);
		divideTe(mid[1], mid[3], c, mid[4], k-1);
		divideTe(mid[2], mid[5], mid[4], d, k-1);
	}
	else
		te(a, b, c, d);
}
...
window.onload = function main(){
	...
	divideTe(vertices[0], vertices[4], vertices[5],
		vertices[6], NumTimesToSubdivide);
