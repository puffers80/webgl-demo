...
var XhourOfDay = 0.0;		// 一天中的小时数，初始为0
var XdayOfYear = 0.0;		// 一年中的天数，初始为0
...
function drawSolarSystem(mvp){
	...
	mvp = mult(mvp, rotateY(60.0 * dayOfYear));
	mvp = mult(mvp, rotateX(90)); // 调整南北极
	mvp = mult(mvp, mult(translate(-1,0,0),scale(0.7, 0.7, 0.7))); // 控制太阳大小的缩放变换	
	...
	mvp = mult(mvp, rotateY(60.0 * dayOfYear));
	mvp = mult(mvp, rotateX(90)); // 调整南北极
	mvp = mult(mvp, mult(translate(1,0,0),scale(0.7, 0.7, 0.7)));
	...
	mvpStack.push(mvp);
    mvp = mult(mvp, rotateY(360.0 * dayOfYear / 365.0));//365
	// 用于控制地球和太阳之间距离的平移变换
    mvp = mult(mvp, translate(4.5, 0.0, 0.0));
	...
	//行星X
	mvpStack.push(mvp);
    mvp = mult(mvp, rotateY(360.0 * XdayOfYear / 300.0));//300
    mvp = mult(mvp, translate(3, 0.0, 0.0));	
	drawXSystem(mvp);
	mvp = mvpStack.pop();
}
...
function drawXSystem(mvp){
    mvpStack.push(mvp);//保存矩阵状态	
	mvp = mult(mvp, rotateY(360.0 * XhourOfDay / 36.0));//自转
	mvp = mult(mvp, rotateX(90)); // 调整南北极
	mvp = mult(mvp, scale(0.55, 0.55, 0.55));//控制大小
	// 画一个球来表示X
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	...
	/*画卫星1+2*/
	// 用dayOfYear来控制其绕行星X的旋转
	mvpStack.push(mvp);
  	mvp = mult(mvp, rotateY(360.0 * XhourOfDay / 36.0));
	// 用于控制地月距离的平移变换
	mvp = mult(mvp, rotateX(90)); // 调整南北极
    mvp = mult(mvp, translate(0.72, 0.0, 0.0));
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));// 控制卫星1大小的缩放变换
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	gl.uniform3f(u_Color, 1.0, 0.7, 0.3 );
	gl.drawArrays(gl.LINES, 0, numVertices);
	mvp = mvpStack.pop();
	//卫星2
	//mvpStack.push(mvp);
	mvp = mult(mvp, rotateY(-360.0 * XdayOfYear / 37.5));//300/8
	mvp = mult(mvp, rotateX(90)); // 调整南北极
    mvp = mult(mvp, translate(1.0, 0.0, 0.0));
	mvp = mult(mvp, scale(0.1, 0.1, 0.1));
	gl.uniformMatrix4fv(u_MVPMatrix, false, flatten(mvp));
	gl.uniform3f(u_Color, 0, 1.0, 1.0);
	gl.drawArrays(gl.LINES, 0, numVertices);
	//mvp = mvpStack.pop();
}
...
function render() {
	...
	mvp = mult(mvp, lookAt(
		vec3(0.0, 2.0, 12.0), // 相机位置
		vec3(0.0, 0.0, 0.0), // 注视点位置
